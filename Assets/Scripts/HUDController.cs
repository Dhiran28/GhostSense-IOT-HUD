...
public class HUDController : MonoBehaviour
{
    ...
    [Header("Waveform")]
    public WaveformController waveform;        // ← ADDED
    ...
    void DoGas()
    {
        ...
        if (gasValueText)
        {
            gasValueText.text = _sm.gasPPM > 999
                ? (_sm.gasPPM / 1000f).ToString("F1") + " k"
                : _sm.gasPPM + " ppm";
        ...
        if (gasStatusText)
        {
            gasStatusText.text = LABELS[_sm.gasLevel];
            gasStatusText.color = c;
        }
        ...
        if (waveform) waveform.AddValue(_sm.gasPPM);  // ← ADDED
    }
    ...
    void DoEnv()
    {
        if (tempText)
        {
            tempText.text = _sm.temp.ToString("F1") + " °C";
            tempText.color = _sm.isTempAlert ? colMedium : colSafe;
        }

        if (humText)
            humText.text = _sm.humidity.ToString("F0") + " %";
    }

    void DoRing()
    {
        Color c = GetColor(_sm.gasLevel);
        string lbl = _sm.isDanger ? "DANGER"
                   : _sm.isMedium ? "CAUTION"
                   : "SAFE";
